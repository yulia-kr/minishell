...
/**
 * @brief   This functions checks if the variable id (name) already exist within the
 *          the env linked list.
 *          If the var id is found within the list then the value is replaced and return 0
 *          else -1 is returned.
 * 
 ...
 * @return int 
 */
static int	ft_is_var_id_n_env_var(char **var_id, char *var, t_parser_var *v_p)
{
	t_list	*head;
	char	*var_n;
...
static int	ft_export_on_parent(char **var_id, char *var, t_parser_var *v_p)
{
	...
	if (ft_is_var_valid(var_id, var, v_p) < 0)
		return (-1);
	if (ft_is_var_id_n_env_var(var_id, var, v_p) == 0)
		return (0);
	var_n = ft_calloc((ft_strlen(var) + 1), SZ_CHAR);
	ft_memcpy((void *)var_n, (void *)var, ft_strlen(var));
