...
/*   Updated: 2022/12/11 10:40:37 by cudoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	ft_run_pipe_cmd_l(int *p, t_cmd *cmd, t_parser_var *v)
{
	if (fork() == 0)
	{
		close(1);
...
static void	ft_run_pipe_cmd_r(int *p, t_cmd *cmd, t_parser_var *v)
{
	if (fork() == 0)
	{
		close(0);
...
void	ft_ms_cmd_pipe(t_cmd *cmd, t_parser_var *v_p)
{
	...
	if (pipe(p) < 0)
		perror("Error!\nsys_pipe failed.");
	ft_run_pipe_cmd_l(p, cmd_pipe->left, v_p);
	ft_run_pipe_cmd_r(p, cmd_pipe->right, v_p);
	close(p[0]);
	close(p[1]);
	wait(&status_l);
	wait(&status_r);
